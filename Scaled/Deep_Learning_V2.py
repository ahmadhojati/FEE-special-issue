...
# The lines below are working for running the code on Borah.
# To run on R2 you just need to change the address on gdal installation and loading your python version
...
spl = 0.7
x_train, x_test, y_train, y_test, train_idx = train_test(data[:,:,:,1:],data[:,:,:,0], spl, 123)
...
x_train, x_val, y_train, y_val, indx= train_test(x_train,y_train, 0.8, 123)
...
loaded_model.compile(optimizer=tf.keras.optimizers.Adam(lr=0.001), loss=tf.keras.losses.mean_squared_error
                    ,  metrics=[det_coeff])
